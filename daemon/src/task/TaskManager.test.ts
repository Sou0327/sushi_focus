...
import type { DaemonEvent, Task } from '../types/index.js';
...
class TestableTaskManager extends TaskManager {
   ...
   * テスト用: タスクの状態を直接設定
   */
  setTaskStatus(status: Task['status']): void {
...
describe('TaskManager', () => {
  let taskManager: TestableTaskManager;
  let broadcastedEvents: DaemonEvent[];

  beforeEach(() => {
    // Use fake timers to avoid real delays in runDemoTask
    vi.useFakeTimers();
    broadcastedEvents = [];
    taskManager = new TestableTaskManager((event: DaemonEvent) => {
